...
#define WINDOW_WIDTH    744//680   // size of window
#define WINDOW_HEIGHT   794//730
...
int Game_Init(void *parms)
{
	...
	Load_Image_Bitmap32(&oct, L"oct.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);
	black_tile.x = 69;
	black_tile.y = 5;
	...
	oct.x = 0;
	oct.y = 0;
	oct.height = TILE_WIDTH;
	oct.width = TILE_WIDTH;
...
int Game_Main(void *parms)
{
	...
	for (int i = 0; i < 100; ++i)
		for (int j = 0; j < 100; ++j)
		{
			...
			if ((tile_x < 0) || (tile_y < 0) || (tile_x >= 100 || tile_y >= 100)) continue;
			if (tile_x % 7 == 0 || tile_y % 7 == 0)
				Draw_Bitmap32(&oct, TILE_WIDTH * i + 7, TILE_WIDTH * j + 7);
