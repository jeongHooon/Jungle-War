	...
	while (1) 
	{
		...
		if (!isStart)
		{
			...
			while (1)
			{
				if (ClientCount > 1) 
				{
					retval = send(client_sock, (char*)&myClientID, sizeof(myClientID), 0);
		...
		else
		{
			...
			if (SRS.ClientID == PLAYER1) memcpy(&P1KeyBuffer, &SRS.KeyBuffer, sizeof(P1KeyBuffer));
			else if (SRS.ClientID == PLAYER2) memcpy(&P2KeyBuffer, &SRS.KeyBuffer, sizeof(P2KeyBuffer));
			...
			if (CalCheck == 0)
			{
				...
				if (P2KeyBuffer[VK_LEFT])
					if (characterPos2.x > 0)
						characterPos2.x -= 100;
				else if (P2KeyBuffer[VK_RIGHT])
					if (characterPos2.x < BOARDSIZE - 100)
						characterPos2.x += 100;
				else if (P2KeyBuffer[VK_DOWN])
					if (characterPos2.y < BOARDSIZE - 100)
						characterPos2.y += 100;
				else if (P2KeyBuffer[VK_UP])
					if (characterPos2.y > 0)
						characterPos2.y -= 100;


				memcpy(&SSS.p1, &characterPos1, sizeof(characterPos1));
				memcpy(&SSS.p2, &characterPos2, sizeof(characterPos2));
