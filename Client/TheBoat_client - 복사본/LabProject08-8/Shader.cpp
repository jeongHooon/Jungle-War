...
CShadowRock* CGameFramework::m_pShadowObject2[];
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CShadowTexturedShader::CShadowTexturedShader()
{
}
...
D3D12_INPUT_LAYOUT_DESC CShadowTexturedShader::CreateInputLayout()
{
	UINT nInputElementDescs = 2;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_BLEND_DESC CShadowTexturedShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = TRUE;
...
D3D12_SHADER_BYTECODE CShadowTexturedShader::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSTextured", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CShadowTexturedShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSTextured", "ps_5_1", ppd3dShaderBlob));
}
...
D3D12_SHADER_BYTECODE CIlluminatedTexturedShader::CreateShadowVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSShadow", "vs_5_1", ppd3dShaderBlob));
}
...
CTreeItemShader::CTreeItemShader() {

}
CTreeItemShader::~CTreeItemShader() {
}

void CTreeItemShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
...
void CTreeItemShader::ReleaseObjects()
{
	if (m_ppBullet)
	{
		for (int j = 0; j < m_nBullet; j++) if (m_ppBullet[j]) delete m_ppBullet[j];
...
void CTreeItemShader::SetPosition(int id, XMFLOAT3 input) {
	m_ppBullet[id]->SetPosition(input);
}

void CTreeItemShader::SetLook(int input, float x, float y, float z) {
	m_ppBullet[input]->SetLookAt(XMFLOAT3(x, y, z));
}

void CTreeItemShader::AnimateObjects(float fTimeElapsed, CCamera *pCamera)
{
	for (int j = 0; j < m_nBullet; j++)
	{
		m_ppBullet[j]->Animate(fTimeElapsed, pCamera);
...
void CTreeItemShader::ReleaseUploadBuffers()
{
	if (m_ppBullet)
	{
		for (int j = 0; j < m_nBullet; j++) if (m_ppBullet[j]) m_ppBullet[j]->ReleaseUploadBuffers();
...
void CTreeItemShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CTexturedShader::Render(pd3dCommandList, pCamera);
...
void CTreeItemShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nBullet, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CTreeItemShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
	for (int j = 0; j < m_nBullet; j++)
	{
...
void CTreeItemShader::ReleaseShaderVariables()
{
	if (m_pd3dcbGameObjects)
	{
		m_pd3dcbGameObjects->Unmap(0, NULL);
...
D3D12_BLEND_DESC CTreeItemShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
void CTreeItemShader::ItemDrop(int id, int count, bool check) {
	if (count < 7)
		m_ppBullet[id]->SetPosition(XMFLOAT3(m_ppBullet[id]->GetPosition().x, m_ppBullet[id]->GetPosition().y + 0.2, m_ppBullet[id]->GetPosition().z));
	else if (count < 15)
		m_ppBullet[id]->SetPosition(XMFLOAT3(m_ppBullet[id]->GetPosition().x, m_ppBullet[id]->GetPosition().y + 0.1, m_ppBullet[id]->GetPosition().z));
...
void CTreeItemShader::SetOOBB(int id, XMFLOAT3 input)
{
	m_ppBullet[id]->SetOOBB(input, XMFLOAT3(3, 3, 3), XMFLOAT4(0, 0, 0, 1));
}

BoundingOrientedBox CTreeItemShader::GetOOBB(int id)
{
	return m_ppBullet[id]->bounding_box;
}
///////////////////////////////////////////////

CDustShader::CDustShader() {

}
CDustShader::~CDustShader() {
}

void CDustShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D_ARRAY, 0);
	//pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/UI/Wood.dds", 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/UI/Dust_1.dds", 0);
...
void CDustShader::SetPosition(int id, XMFLOAT3 input) {
	m_ppBullet[id]->SetPosition(input);
}

void CDustShader::SetDustPosition(XMFLOAT3 input) {
	for(int i=0;i<10;++i)
		m_ppBullet[i]->SetPosition(input.x, input.y + i, input.z);
	for (int i = 0; i<10; ++i)
		m_ppBullet[i + 10]->SetPosition(input.x + 5, input.y + i, input.z + 5);
}

void CDustShader::SetLook(int input, float x, float y, float z) {
	m_ppBullet[input]->SetLookAt(XMFLOAT3(x, y, z));
}

void CDustShader::AnimateObjects(float fTimeElapsed, CCamera *pCamera)
{
	for (int j = 0; j < m_nBullet; j++)
	{
		m_ppBullet[j]->Animate(fTimeElapsed, pCamera);
...
void CDustShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CTexturedShader::Render(pd3dCommandList, pCamera);
...
void CDustShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nBullet, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CDustShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
	for (int j = 0; j < m_nBullet; j++)
	{
...
void CDustShader::ReleaseShaderVariables()
{
	if (m_pd3dcbGameObjects)
	{
		m_pd3dcbGameObjects->Unmap(0, NULL);
...
D3D12_BLEND_DESC CDustShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
void CDustShader::ItemDrop(int id, int count, bool check) {
	if (count < 7)
		m_ppBullet[id]->SetPosition(XMFLOAT3(m_ppBullet[id]->GetPosition().x, m_ppBullet[id]->GetPosition().y + 0.2, m_ppBullet[id]->GetPosition().z));
	else if (count < 15)
		m_ppBullet[id]->SetPosition(XMFLOAT3(m_ppBullet[id]->GetPosition().x, m_ppBullet[id]->GetPosition().y + 0.1, m_ppBullet[id]->GetPosition().z));
...
void CDustShader::SetOOBB(int id, XMFLOAT3 input)
{
	m_ppBullet[id]->SetOOBB(input, XMFLOAT3(3, 3, 3), XMFLOAT4(0, 0, 0, 1));
}

BoundingOrientedBox CDustShader::GetOOBB(int id)
{
	return m_ppBullet[id]->bounding_box;
}

////////////////////////////////////////////////////
CBulletItemShader::CBulletItemShader() {

}
CBulletItemShader::~CBulletItemShader() {
}

void CBulletItemShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Items/BulletItem.dds", 0);
...
void CBulletItemShader::ReleaseObjects()
{
	if (m_ppBullet)
	{
		for (int j = 0; j < m_nBullet; j++) if (m_ppBullet[j]) delete m_ppBullet[j];
...
void CBulletItemShader::SetPosition(int id, XMFLOAT3 input) {
	m_ppBullet[id]->SetPosition(input);
}

void CBulletItemShader::SetLook(int input, float x, float y, float z) {
	m_ppBullet[input]->SetLookAt(XMFLOAT3(x, y, z));
}

void CBulletItemShader::AnimateObjects(float fTimeElapsed, CCamera *pCamera)
{
	for (int j = 0; j < m_nBullet; j++)
	{
		m_ppBullet[j]->Animate(fTimeElapsed, pCamera);
...
void CBulletItemShader::ReleaseUploadBuffers()
{
	if (m_ppBullet)
	{
		for (int j = 0; j < m_nBullet; j++) if (m_ppBullet[j]) m_ppBullet[j]->ReleaseUploadBuffers();
...
void CBulletItemShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CTexturedShader::Render(pd3dCommandList, pCamera);
...
void CBulletItemShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nBullet, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CBulletItemShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
	for (int j = 0; j < m_nBullet; j++)
	{
...
void CBulletItemShader::ReleaseShaderVariables()
{
	if (m_pd3dcbGameObjects)
	{
		m_pd3dcbGameObjects->Unmap(0, NULL);
...
D3D12_BLEND_DESC CBulletItemShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
void CBulletItemShader::ItemDrop(int id, int count, bool check) {
	if (count < 7)
		m_ppBullet[id]->SetPosition(XMFLOAT3(m_ppBullet[id]->GetPosition().x, m_ppBullet[id]->GetPosition().y + 0.2, m_ppBullet[id]->GetPosition().z));
	else if (count < 15)
		m_ppBullet[id]->SetPosition(XMFLOAT3(m_ppBullet[id]->GetPosition().x, m_ppBullet[id]->GetPosition().y + 0.1, m_ppBullet[id]->GetPosition().z));
...
void CBulletItemShader::SetOOBB(int id, XMFLOAT3 input)
{
	m_ppBullet[id]->SetOOBB(input, XMFLOAT3(3, 3, 3), XMFLOAT4(0, 0, 0, 1));
}

BoundingOrientedBox CBulletItemShader::GetOOBB(int id)
{
	return m_ppBullet[id]->bounding_box;
}
...
void CParticleShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Blood/BloodEffect.dds", 0);
	...
	CMainScreenMesh *pCubeMesh = new CMainScreenMesh(pd3dDevice, pd3dCommandList, 1, 0.3, 0.4);
...
void CDropItemShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	CMainScreenMesh* pCubeMesh = new CMainScreenMesh(pd3dDevice, pd3dCommandList, 10.0, 10.0, 10.0);
...
CGameWinShader::CGameWinShader() {
}
CGameWinShader::~CGameWinShader() {
}


D3D12_DEPTH_STENCIL_DESC CGameWinShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = FALSE;
...
D3D12_SHADER_BYTECODE CGameWinShader::CreateVertexShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VS_UI", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CGameWinShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PS_UI", "ps_5_1", ppd3dShaderBlob));
}

void CGameWinShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/UI/YouWin.dds", 0);
...
void CGameWinShader::ReleaseObjects()
{
	if (m_ppTree)
	{
		for (int j = 0; j < m_nTree; j++) if (m_ppTree[j]) delete m_ppTree[j];
...
void CGameWinShader::AnimateObjects(float fTimeElapsed, CCamera *pCamera)
{
	for (int j = 0; j < m_nTree; j++)
	{
		m_ppTree[j]->Animate(fTimeElapsed);
...
void CGameWinShader::ReleaseUploadBuffers()
{
	if (m_ppTree)
	{
		for (int j = 0; j < m_nTree; j++) if (m_ppTree[j]) m_ppTree[j]->ReleaseUploadBuffers();
...
void CGameWinShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CTexturedShader::Render(pd3dCommandList, pCamera);
...
void CGameWinShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nTree, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CGameWinShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
}

void CGameWinShader::ReleaseShaderVariables()
{
	if (m_pd3dcbGameObjects)
	{
		m_pd3dcbGameObjects->Unmap(0, NULL);
...
D3D12_BLEND_DESC CGameWinShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
CBloodScreenShader::CBloodScreenShader() {
}
CBloodScreenShader::~CBloodScreenShader() {
}

D3D12_DEPTH_STENCIL_DESC CBloodScreenShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = FALSE;
...
D3D12_SHADER_BYTECODE CBloodScreenShader::CreateVertexShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VS_UI", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CBloodScreenShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PS_UI", "ps_5_1", ppd3dShaderBlob));
}

void CBloodScreenShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
...
void CBloodScreenShader::ReleaseObjects()
{
	if (m_ppTree)
	{
		for (int j = 0; j < m_nTree; j++) if (m_ppTree[j]) delete m_ppTree[j];
...
void CBloodScreenShader::AnimateObjects(float fTimeElapsed, CCamera *pCamera)
{
	for (int j = 0; j < m_nTree; j++)
	{
		m_ppTree[j]->Animate(fTimeElapsed);
...
void CBloodScreenShader::ReleaseUploadBuffers()
{
	if (m_ppTree)
	{
		for (int j = 0; j < m_nTree; j++) if (m_ppTree[j]) m_ppTree[j]->ReleaseUploadBuffers();
...
void CBloodScreenShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CTexturedShader::Render(pd3dCommandList, pCamera);
...
void CBloodScreenShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nTree, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CBloodScreenShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
}

void CBloodScreenShader::ReleaseShaderVariables()
{
	if (m_pd3dcbGameObjects)
	{
		m_pd3dcbGameObjects->Unmap(0, NULL);
...
D3D12_BLEND_DESC CBloodScreenShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
CBloodScreenShader_1::CBloodScreenShader_1() {
}
CBloodScreenShader_1::~CBloodScreenShader_1() {
}

D3D12_DEPTH_STENCIL_DESC CBloodScreenShader_1::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = FALSE;
...
D3D12_SHADER_BYTECODE CBloodScreenShader_1::CreateVertexShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VS_UI", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CBloodScreenShader_1::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PS_UI", "ps_5_1", ppd3dShaderBlob));
}

void CBloodScreenShader_1::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Blood/BloodEffect_1.dds", 0);
...
void CBloodScreenShader_1::ReleaseObjects()
{
	if (m_ppTree)
	{
		for (int j = 0; j < m_nTree; j++) if (m_ppTree[j]) delete m_ppTree[j];
...
void CBloodScreenShader_1::AnimateObjects(float fTimeElapsed, CCamera *pCamera)
{
	for (int j = 0; j < m_nTree; j++)
	{
		m_ppTree[j]->Animate(fTimeElapsed);
...
void CBloodScreenShader_1::ReleaseUploadBuffers()
{
	if (m_ppTree)
	{
		for (int j = 0; j < m_nTree; j++) if (m_ppTree[j]) m_ppTree[j]->ReleaseUploadBuffers();
...
void CBloodScreenShader_1::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CTexturedShader::Render(pd3dCommandList, pCamera);
...
void CBloodScreenShader_1::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nTree, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CBloodScreenShader_1::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
}

void CBloodScreenShader_1::ReleaseShaderVariables()
{
	if (m_pd3dcbGameObjects)
	{
		m_pd3dcbGameObjects->Unmap(0, NULL);
...
D3D12_BLEND_DESC CBloodScreenShader_1::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
