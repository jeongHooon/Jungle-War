			...
			for (int i = 0; i < MAX_PLAYER_SIZE; ++i) {
				...
				if (clients[i].is_move_left) {
					if (clients[i].is_running && clients[i].CType != TYPE_SPEED) {
						...
						clients[i].z += clients[i].look_vec.x * (RUN_SPEED * BOOST_SPEED * overlapped_buffer->elapsed_time) / METER_PER_PIXEL;
						clients[i].x += (-1) * clients[i].look_vec.z * (RUN_SPEED * BOOST_SPEED * overlapped_buffer->elapsed_time) / METER_PER_PIXEL;
					}
					else {
				...
				if (clients[i].is_move_right) {
					if (clients[i].is_running && clients[i].CType != TYPE_SPEED) {
						...
						clients[i].z += (-1) * clients[i].look_vec.x * (RUN_SPEED * BOOST_SPEED * overlapped_buffer->elapsed_time) / METER_PER_PIXEL;
						clients[i].x += clients[i].look_vec.z * (RUN_SPEED * BOOST_SPEED * overlapped_buffer->elapsed_time) / METER_PER_PIXEL;
					}
					else {
...
>>>>>>> bda2dac26d67e840d99061561328edeee1903dc1
