...
void CGameFramework::CreateSwapChain()
{
	...
	//전체화면 모드에서 바탕화면의 해상도를 바꾸지 않고 후면버퍼의 크기를 바탕화면 크기로 변경한다. dxgiSwapChainDesc.Flags = 0;
#else
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	// -----------------------------------------------------------------------------------
	// 용빠 PDF에서는 OnResizeBackBuffer에서 하라고 됭어ㅣㅆ는뎅...
	// 아닌거같음
	...
	wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_UP:
			...
			break;
		case VK_DOWN:
			network_mgr->ReleaseDownandSend();
			break;
		case VK_RIGHT:
			network_mgr->ReleaseRightandSend();
			break;
		case VK_LEFT:
			network_mgr->ReleaseLeftandSend();
			break;
	...
	WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		...
		OnResizeBackBuffers();	// 윈도우의 크기가 변경되면 후면버퍼의 크기를 변경한다. 
